...
// Capture unexpected failures and store diagnostics
process.on('unhandledRejection', async (reason) => {
    try {
        await Actor.setValue('UNHANDLED_REJECTION', String(reason));
    } catch {}
...
process.on('uncaughtException', async (err) => {
    try {
        await Actor.setValue('UNCAUGHT_EXCEPTION', { message: err?.message, stack: err?.stack });
    } catch {}
    log.error(`Uncaught exception: ${err?.stack || err?.message || String(err)}`);
});
...
Actor.main(async () => {
    ...
    try {
        ...
        log.error(`Run failed: ${err?.message || String(err)}`);
        await Actor.setValue('RUN_ERROR', { message: err?.message || String(err), stack: err?.stack, lastUrl });
        throw err; // ensure actor fails visibly in Apify UI
    }
});